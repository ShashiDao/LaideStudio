## Current State
- Phase: [SESSION-MEMORY-03]
- Last verified working: Implemented Session Memory for persistent learned insights across tasks within a project. Added private `sessionMemory` table in Dexie (`src/db.ts`, schema version 9) with `{ id, projectId, insight, key?, sourceTaskId?, sourceTaskPrompt?, timestamp }`. Created `src/services/agent/sessionMemory.ts` with bounded table retention (rotates oldest entries when exceeding `DEFAULT_MAX_SESSION_MEMORIES = 30`), chronological retrieval, and markdown bullet formatting for prompt injection. Updated `buildSystemPrompt()` in `src/services/agent/prompts.ts` to inject a `<session_memory>` block alongside `<custom_instructions>` and `<skills>`. Integrated session memory reading in `ChatPanel.tsx` and `agentLoop.ts`. Hooked session memory writes directly into the durable task lifecycle completion points in `src/services/agent/task/taskStore.ts` (`updateTaskState` transitioning to `verified`/`completed` and `finishRun` with `completed` status), with deduplication by `sourceTaskId`. Confirmed `sessionMemory` is a private Dexie table that never leaks into VFS `listFiles`, ZIP exports, or GitHub pushes. Added comprehensive test coverage in `src/services/agent/sessionMemory.test.ts` and `src/services/agent/prompts.test.ts`. All 21 test suites (185 tests) pass, linter passes with 0 errors, and applet compilation succeeds.
...
### 2026-09-04
- **Session Memory, Bounded Insight Retention & Task Lifecycle Prompt Injection** (FEATURE-03): Added a private `sessionMemory` table in Dexie (`src/db.ts`, schema version 9) with `{ id, projectId, insight, key?, sourceTaskId?, sourceTaskPrompt?, timestamp }`. Implemented `src/services/agent/sessionMemory.ts` with bounded table retention (rotates oldest entries when exceeding `DEFAULT_MAX_SESSION_MEMORIES = 30`), chronological retrieval, and markdown bullet formatting for prompt injection. Updated `buildSystemPrompt()` in `src/services/agent/prompts.ts` to inject a `<session_memory>` block alongside `<custom_instructions>` and `<skills>`. Integrated session memory reading into `ChatPanel.tsx` and `agentLoop.ts`. Hooked session memory writes directly into the durable task lifecycle completion points in `src/services/agent/task/taskStore.ts` (`updateTaskState` transitioning to `verified`/`completed` and `finishRun` with `completed` status), with deduplication by `sourceTaskId`. Confirmed `sessionMemory` is a private Dexie table that never leaks into VFS `listFiles`, ZIP exports, or GitHub pushes. Added test coverage in `src/services/agent/sessionMemory.test.ts` and `src/services/agent/prompts.test.ts`.
- **Fresh-Context Reviewer Integration & Advisory Patch Critique** (FEATURE-02): Created `src/services/agent/review.ts` exporting `runFreshContextReview(patches, userRequest, adapter, signal, usageContext)`. Uses a single non-streaming `adapter.send()` call with zero prior chat history, an adversarial system prompt actively hunting for correctness, security, and styling flaws, and tolerant regex JSON extraction for `{ findings: [{ severity, file, summary }] }`. Added `fresh_context_review` category to `src/services/usage/tokenSpend.ts`. Integrated reviewer call into `src/services/agent/agentLoop.ts` after `WorkspaceOverlay` verification succeeds and before patches reach the UI. Stored advisory findings in `PatchSlice` in `src/store.ts`. Added a collapsible "Reviewer Critique" section to `src/components/chat/PatchReviewSheet.tsx` with severity count chips and finding cards, strictly advisory and never blocking patch approval or triggering autonomous repair. Added test coverage in `src/services/agent/review.test.ts`, `src/components/chat/PatchReviewSheet.test.ts`, and `src/services/agent/agentLoop.test.ts`.
- **Skills Library Integration, Keyword Matching & Token-Budgeted Prompt Injection** (FEATURE-01): Added a global skills table to Dexie database (`src/db.ts`, schema version 8) with `{ id, name, description, keywords: string[], content: string, createdAt: number, updatedAt: number }`. Implemented `src/services/agent/skills.ts` with default seeded skills (Tailwind conventions, Vitest testing guidelines, VFS overlay safety), keyword/substring matching with stopword rejection and relevance scoring, and token-budgeted prompt injection capping combined skills to `DEFAULT_SKILLS_TOKEN_BUDGET = 1500` tokens using `countTokensForText`. Updated `buildSystemPrompt()` (`src/services/agent/prompts.ts`) to inject a `<skills>` block matching the mechanism used by `<custom_instructions>`. Integrated skills matching in `ChatPanel.tsx` and `agentLoop.ts`. Built `SettingsSkillsSection.tsx` embedded in `SettingsAIProvidersTab.tsx` providing full CRUD operations, keyword tags, token estimation badges, markdown preview accordions, and default restoration. Added comprehensive unit and UI tests in `src/services/agent/skills.test.ts`, `src/services/agent/prompts.test.ts`, and `src/components/shared/SettingsSkillsSection.test.tsx`.
