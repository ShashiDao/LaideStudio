## Current State
- Phase: HOTFIX-103
- Last verified working: Refactored SettingsPanel.tsx into section components under src/components/shared/settings/ (ThemeSection, ProviderProfilesSection, McpServersSection, DeploySection, UsageSection, SecuritySection, DependencyCacheSection, ShortcutsSection). SettingsPanel.tsx is now a thin composition shell. All 77 test suites (569 tests) pass, linter and production build clean with 0 errors.
...
### [HOTFIX-99] Expand Icon-Only Button Hit Areas to 44x44px — 2026-08-31
Prompt: Increase hit area of icon-only buttons in TopStrip and ProjectFilesPane to >=44x44px with proper spacing and tests.
Files touched:
- `src/components/shared/TopStrip.tsx` (modified)
- `src/components/project/ProjectFilesPane.tsx` (modified)
- `src/components/shared/TopStrip.test.tsx` (modified)
- `src/components/project/ProjectFilesPane.test.tsx` (new)
- `AI_CHANGELOG.md` (modified)
Changed:
- Updated Keyboard Shortcuts, Theme Toggle, and Lock Vault buttons in `TopStrip.tsx` to `min-w-[44px] min-h-[44px]`, preserving visible icon sizes (12-13px) and sleek header density (`min-h-[44px] px-3 py-1`).
- Expanded Create New Project and Archive buttons in `ProjectFilesPane.tsx` to `min-w-[44px] min-h-[44px] p-2` within the `min-h-[44px]` project explorer header.
- Widened spacing between adjacent action buttons (`gap-2 sm:gap-2.5`) to prevent accidental mis-taps between Theme Toggle and Lock Vault.
- Added test coverage in `TopStrip.test.tsx` and `ProjectFilesPane.test.tsx` asserting the 44x44px minimum touch targets.
Decisions:
- Used `min-w-[44px] min-h-[44px]` directly on button elements with centered flexbox alignment so hit areas meet mobile accessibility standards without visual distortion.
- Kept the outer header containers compact (`min-h-[44px]`) to maintain existing workspace proportions.
Deviations: none
Verified: All 76 test suites (568 tests) pass via `npx vitest run`; `lint_applet` passed with 0 errors; `compile_applet` build succeeded.
Open questions: none

### [HOTFIX-100] Extract bundleHtml Service Module — 2026-08-31
Prompt: Extract buildBundledHtml, detectProjectTailwindVersion, and injectTailwindScriptIntoHtml from PreviewPanel.tsx to src/services/bundler/bundleHtml.ts and update references.
Files touched:
- `src/services/bundler/bundleHtml.ts` (new)
- `src/components/preview/PreviewPanel.tsx` (modified)
- `src/services/deploy/deployClient.ts` (modified)
- `src/services/bundler/esbuild.worker.test.ts` (modified)
- `AI_CHANGELOG.md` (modified)
Changed:
- Extracted `buildBundledHtml`, `detectProjectTailwindVersion`, and `injectTailwindScriptIntoHtml` from `PreviewPanel.tsx` into standalone domain module `src/services/bundler/bundleHtml.ts`.
- Updated `src/components/preview/PreviewPanel.tsx` to import and re-export the three functions from `bundleHtml.ts`.
- Updated `src/services/deploy/deployClient.ts` and `src/services/bundler/esbuild.worker.test.ts` to import directly from `../bundler/bundleHtml` and `./bundleHtml`.
- Preserved exact runtime behavior, signatures, and test coverage without modifying any test assertions.
Decisions:
- Re-exported the three functions from `PreviewPanel.tsx` to maintain complete backward compatibility with any other modules or tests.
Deviations: none
Verified: Full test suite passing (76 test suites, 568 tests); `lint_applet` passed with 0 errors; `compile_applet` build succeeded.
Open questions: none

### [HOTFIX-101] Clean Up PreviewPanel Imports and Inspect Mode Error Logging — 2026-08-31
Prompt: Remove unused ChevronUp import from PreviewPanel.tsx and add console.debug in handleIframeLoad catch block for inspect mode.
Files touched:
- `src/components/preview/PreviewPanel.tsx` (modified)
- `AI_CHANGELOG.md` (modified)
Changed:
- Removed unused `ChevronUp` import from `lucide-react` in `src/components/preview/PreviewPanel.tsx`.
- Added `console.debug('Failed to toggle inspect mode', err)` in `handleIframeLoad` catch block when `postMessage` fails.
Decisions:
- Used `console.debug` to keep dev tools output informative without triggering visual error noise in production preview.
Deviations: none
Verified: `npx vitest run src/components/preview/PreviewPanel.test.tsx` passed (12/12); `lint_applet` passed with 0 errors; `compile_applet` build succeeded.
Open questions: none

### [HOTFIX-102] Extract Provider Taglines to providerMeta Module — 2026-08-31
Prompt: Extract provider flagship model taglines from SettingsPanel.tsx into exported PROVIDER_TAGLINES in src/services/llm/providerMeta.ts and reference it.
Files touched:
- `src/services/llm/providerMeta.ts` (new)
- `src/services/llm/providerMeta.test.ts` (new)
- `src/components/shared/SettingsPanel.tsx` (modified)
- `AI_CHANGELOG.md` (modified)
Changed:
- Created `src/services/llm/providerMeta.ts` exporting `PROVIDER_TAGLINES: Record<string, string>` mapping provider IDs (`anthropic`, `openai`, `google`, `openrouter`, `openai-compatible`) to their display taglines (`Claude 3.7`, `GPT-4o`, `Gemini 1.5/2.0`, `400+ Models`, `Ollama / Local`).
- Updated `src/components/shared/SettingsPanel.tsx` to import `PROVIDER_TAGLINES` and render `{PROVIDER_TAGLINES[opt.id] || ''}` within the provider picker button cards.
- Added unit tests in `src/services/llm/providerMeta.test.ts` verifying all default provider mappings.
Decisions:
- Maintained fallback empty string `|| ''` to prevent rendering undefined if unknown custom provider IDs are encountered in future profiles.
Deviations: none
Verified: All tests passing (`npx vitest run src/components/shared/SettingsPanel.test.tsx src/services/llm/providerMeta.test.ts`); `lint_applet` passed with 0 errors; `compile_applet` build succeeded.
Open questions: none

### [HOTFIX-103] Refactor SettingsPanel into Modular Section Components — 2026-08-31
Prompt: Refactor 2,200+ line monolithic SettingsPanel.tsx into section components under src/components/shared/settings/, with SettingsPanel.tsx becoming a thin composition shell.
Files touched:
- `src/components/shared/settings/ThemeSection.tsx` (new)
- `src/components/shared/settings/ProviderProfilesSection.tsx` (new)
- `src/components/shared/settings/DeploySection.tsx` (new)
- `src/components/shared/settings/McpServersSection.tsx` (new)
- `src/components/shared/settings/UsageSection.tsx` (new)
- `src/components/shared/settings/SecuritySection.tsx` (new)
- `src/components/shared/settings/DependencyCacheSection.tsx` (new)
- `src/components/shared/settings/ShortcutsSection.tsx` (new)
- `src/components/shared/SettingsPanel.tsx` (modified)
- `AI_CHANGELOG.md` (modified)
Changed:
- Extracted domain-specific settings logic and state into focused section components under `src/components/shared/settings/`: `ThemeSection` (theme modes, palette swatches, contrast slider), `ProviderProfilesSection` (connection profiles, LLM models, base URL, API keys, ensemble configuration, custom instructions), `DeploySection` (GitHub PAT, Netlify/Vercel/Cloudflare deploy tokens), `McpServersSection` (MCP servers SSE endpoint management), `UsageSection` (database diagnostics, token context window gauge, session spend summary), `SecuritySection` (vault lock, unreviewed patch warning modal, encrypted backup export/restore), `DependencyCacheSection` (npm module cache counters and purge), and `ShortcutsSection` (keyboard accelerator reference accordion).
- Transformed `SettingsPanel.tsx` into a thin composition shell (~200 lines) managing only container responsive width observation (`isWide`), active category routing, and the single-column/two-column outer layout frames.
- Preserved all prop contracts, accessibility aria attributes, test IDs, and visual appearances exactly.
Decisions:
- Encapsulated section-specific local state (e.g. form inputs, validation modals, temporary status messages) within each section component, while keeping global state access bound to `useAppStore` and Dexie database helpers.
Deviations: none
Verified: All 77 test suites (569 tests) passed cleanly via `npm test`; `lint_applet` passed with 0 errors; `compile_applet` build succeeded.
Open questions: none
