...
describe('previewCapture', () => {
  it('injects capture script immediately after opening head tag', () => {
    ...
    expect(result).toContain('laide-preview-capture-helper');
    expect(result).toContain(INJECTED_PREVIEW_CAPTURE_SCRIPT);
    expect(result.indexOf('laide-preview-capture-helper')).toBeGreaterThan(result.indexOf('<head>'));
    expect(result.indexOf('laide-preview-capture-helper')).toBeLessThan(result.indexOf('<title>'));
  });
  ...
  it('injects capture script immediately after opening html tag if no head tag', () => {
    ...
    const result = injectCaptureScriptIntoHtml(html);
    expect(result).toContain('laide-preview-capture-helper');
    expect(result.indexOf('laide-preview-capture-helper')).toBeGreaterThan(result.indexOf('<html>'));
    expect(result.indexOf('laide-preview-capture-helper')).toBeLessThan(result.indexOf('<body>'));
  });
  ...
  it('prepends capture script if no structural tags exist', () => {
    ...
    const result = injectCaptureScriptIntoHtml(html);
    expect(result).toContain('laide-preview-capture-helper');
    expect(result.indexOf('laide-preview-capture-helper')).toBeLessThan(result.indexOf('<div>Fragment</div>'));
  });
  ...
  it('injects helper script before other scripts in head', () => {
    ...
    const result = injectCaptureScriptIntoHtml(html);
    expect(result.indexOf('laide-preview-capture-helper')).toBeLessThan(result.indexOf('other.js'));
  });
});
