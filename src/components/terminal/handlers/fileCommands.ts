  ...
  switch (command) {
    ...
    case 'cat': {
      ...
      for (const rawFile of args.filter(arg => arg !== '-n')) {
        const found = findFile(context, context.resolvePath(cwd, rawFile));
    ...
    case 'grep': {
      ...
      const searchTargets = targetFile ? files.filter(file => file.path === context.resolvePath(cwd, targetFile)) : files;
      if (targetFile && searchTargets.length === 0) return { outputType: 'stderr', outputText: `grep: ${targetFile}: No such file or directory` };
    ...
    case 'wc': {
      ...
      const targetFiles = fileArgs.length ? files.filter(file => fileArgs.some(arg => context.resolvePath(cwd, arg) === file.path)) : files;
