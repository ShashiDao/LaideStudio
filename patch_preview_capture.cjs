...
content = content.replace(
  'export const INJECTED_PREVIEW_CAPTURE_SCRIPT = `(function() {\n  if (window.__XIOM_CAPTURE_INITIALIZED__) return;\n  window.__XIOM_CAPTURE_INITIALIZED__ = true;',
  `export const INJECTED_PREVIEW_CAPTURE_SCRIPT = \`\n${shimAndErrorHandler}\n(function() {\n  if (window.__XIOM_CAPTURE_INITIALIZED__) return;\n  window.__XIOM_CAPTURE_INITIALIZED__ = true;`
);

// Fix injection point to be prepended
const newInjectionLogic = `export function injectCaptureScriptIntoHtml(html: string): string {
...
content = content.replace(
  /export function injectCaptureScriptIntoHtml.*?return `\$\{html\}\\n\$\{scriptTag\}`;/s,
  newInjectionLogic
);
